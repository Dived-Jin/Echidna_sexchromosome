...
use strict;
use Getopt::Long;
my ($blast_result, @fasta, $query_alignRate_cutoff, $subject_alignRate_cutoff, $identity_cutoff, $best, $better);
my ($Verbose,$Help);
GetOptions(
	"i:s"=>\$blast_result,
	"f:s"=>\@fasta,
...
Usage:
	...
	-better <FLOAT> retain the best several hits that score is [0~1] of the best score, default=0
...
$better = defined($better) ? $better : 0;
#################################################################################################################
...
foreach my $q_id (sort keys %result) {
	...
	foreach my $p (sort {$b->[-1] <=> $a->[-1] or $b->[1] <=> $a->[1]} @{$result{$q_id}}) {
		...
		$best_score = $score if($best_score == 0);
		if($score >= $better*$best_score){
